...
app.directive("appMap", function () {
    return {
        ...
        scope: {
            searchQuery: "@",
            searchMarker: "@",  // Map marker for search result (e.g. <code>{ latitude: 10, longitude: 10 }</code>)
        ...
        link: function (scope, element, attrs) {
            ...
            scope.$watch("center", function () {
                if (map && scope.center){
                    ...
                    scope.searchMarker = scope.center;
                    createSearchMarker();
            ...
            function createSearchMarker(){
                ...
                if (map && scope.searchMarker) {
                    var iconBase = 'img/grabrev.png';
                    // create new marker
                    var marker = scope.searchMarker;
                    var loc = new google.maps.LatLng(marker.lat, marker.lon);
                    searchMarker = new google.maps.Marker({ position: loc, map: map, icon: iconBase,title: scope.searchQuery });
                }    
            }
